...
var status = automation_application_msgs.RobotStatus.Constants.IDLE;
var battery_voltage = 28;
var MIN_VOLTAGE_THERSHOLD = 24.5;
var last_voltage_detected;
...
function find_closest_charging_port(current_pose){
  ...
  for(var i=0;i<routePlanner.goalList.length;i++){
    ...
    if(targets.get_target_by_id(goal.id).charging_port == true){
      ...
      req.goal = targets[goal.pointID];
      req.tolerance = 0.15;
      if(goal.type == "charging_port"){
          ...
          .then((res) => {
              ...
              for(let i=1; i<res.plan.poses.length; i++){
                  let x1 = res.plan.poses[0].pose.position.x;
                  let y1 = res.plan.poses[0].pose.position.y;
                  let x2 = res.plan.poses[i].pose.position.x;
                  let y2 = res.plan.poses[i].pose.position.y;
                  totalLen += (x2-x1)*(x2-x1)+(y2-y1)*(y2-y1);
                  lastPoseX = curPoseX; 
                  lastPoseY = curPoseY;
              }
...
async function handleNonNavigationAction(action) {
  switch (action.type) {
    case automation_application_msgs.WaypointType.Constants.SLEEP:
      ...
      await new Promise(resolve => setTimeout(resolve, action.sleep_time_in_sec * 1000));
      break;
...
io.on('connection', function (socket) {
  ...
  socket.on('client_continue_route', function () {
  ...
  .then((rosNode) => {
    ...
    setInterval(function () {
      let battery_voltage_subscriber = rosNode.subscribe('/scout_status', 'scout_msgs/ScoutStatus',
        (data) => {
          last_voltage_detected = battery_voltage;
          battery_voltage = data.battery_voltage;
          if (status == automation_application_msgs.RobotStatus.Constants.CHARGING && battery_voltage < last_voltage_detected) {
    ...
    autoCharge_actionClient._acInterface.on('result', (data) => {
      console.log(data.status);
      if (data.status.status == 3) {
        status = automation_application_msgs.RobotStatus.Constants.CHARGING;
        let recharge_timer = setTimeout(async () => {
          console.log("in interval");
          status = automation_application_msgs.RobotStatus.Constants.CHARGED;
          let start_time = Date.now();
          async function loopCmdVel() {
            let rate = 10;
          ...
          await loopCmdVel();
          sendCmdVel(0, 0);
          status = automation_application_msgs.RobotStatus.Constants.IDLE;
          clearTimeout(recharge_timer);
        }, 300000); // 50 minutes
      }
      if (data.status.status == 4 || data.status.status == 2) {
        status = automation_application_msgs.RobotStatus.Constants.CHARGING_ABORTED;
        let transform = tfTree.lookup_transform('map', 'base_link', 0);
        let cp = find_closest_charging_port(transform);
        if(cp == -1)
          socket.emit('no cp in route');
        else 
    ...
    moveBase_actionClient._acInterface.on('result', async (data) => {
      ...
      if (routePlanner.isRouteActive()) {
        ...
        if (data.status.status == 3 || data.status.status == 4) {
          ...
          if (!(await checkCurTimeInTable()) || battery_voltage <= MIN_VOLTAGE_THERSHOLD) {
          ...
          } else if (robot_status == automation_application_msgs.RobotStatus.Constants.GOING_HOME) {
            if (data.status.status == 3) {
